...
namespace Goliath.Web.FormAuth
{
    ...
    public static class FormsAuthentication
    {
        ...
        public const string SessionCookieName = "gth_sx";
        ...
        public static Response UserLoggedInRedirectResponse(NancyContext context, string userIdentifier, DateTime? cookieExpiry = null, string fallbackRedirectUrl = null)
        {
            ...
            var sessionId = GetSessionId(userIdentifier);
            var sessionCookie = BuildSessionCookie(sessionId, currentConfiguration);

            return response.WithCookie(sessionCookie).WithCookie(authenticationCookie);
        }


        private static string GetSessionId(string userIdentifier)
        {
            var split = userIdentifier.Split(new string[] {"|"}, StringSplitOptions.RemoveEmptyEntries);

            if (split.Length < 2)
                throw new SecurityException("user identifier should be in correct format.");
        ...
        private static Func<NancyContext, Response> GetLoadAuthenticationHook(FormsAuthenticationConfiguration configuration)
        {
            ...
            return context =>
                   {
                       var authCookieValue = GetAuthenticatedUserFromCookie(context, configuration);

                       if (!string.IsNullOrWhiteSpace(authCookieValue))
                       {
                           context.CurrentUser = configuration.UserMapper.GetUserFromIdentifier(authCookieValue, context);
                       }
        ...
        public static string GetSessionFromCookie(NancyContext context)
        {
            ...
            var cookieValue = DecryptAndValidateAuthenticationCookie(context.Request.Cookies[SessionCookieName], currentConfiguration);
        ...
        {
            var cookieContents = EncryptAndSignCookie(userIdentifier, configuration);
        ...
        public static INancyCookie BuildSessionCookie(string sessionId, FormsAuthenticationConfiguration configuration)
        {
            if (configuration == null)
                configuration = currentConfiguration;

            var cookieContents = EncryptAndSignCookie(sessionId, configuration);

            var cookie = new NancyCookie(SessionCookieName, cookieContents, true, configuration.RequiresSSL) {Expires = DateTime.UtcNow.AddMinutes(15)};
